...
public static function isValidUserName(?string $name) : bool{
	...
	public function sendPing() {
		if ($this->ping <= 100) {
			$this->sendMessage("§7[§6Пинг§7] §fОтличное подключение§e ({$this->ping}ms)");
		} elseif ($this->ping <= 200) {
			$this->sendMessage("§7[§6Пинг§7] §fХорошее подключение§e ({$this->ping}ms)");
		} else {
			$this->sendMessage("§7[§6Пинг§7] §fУжасное подключение§e ({$this->ping}ms)");
		}
	}
  ...
  if ($this->isLogin)
 $this->getServer()->getNetwork()->lockAddress($this->getAddress(), 300);
 $this->isLogin = true;
		if(!$this->server->isWhitelisted($this->getLowerCaseName())){
			$this->close($this->getLeaveMessage(), "§l§6Сервер обновляется!");
				...
				if($packet->inputJson !== null){
					if (is_countable($packet->inputJson)) {
					    if(count($packet->inputJson) > 15){
						    $this->server->getNetwork()->blockAddress($this->getAddress(), 600);
						    $this->close("", "", false);
						    return;
						}
					}
	...
	protected function callDeathEvent(){
		...
		if($this->server->expEnabled and !$ev->getKeepExperience()){
			...
			$this->getLevel()->spawnXPOrb($this->add(0, 0.4, 0), $exp);
			$this->removeAllEffects();
