...
public static function generateChunkLoaderId(loadchunk\ChunkLoader $loader): int {
    ...
    public function __construct(Server $server, string $name, string $path, string $provider) {
        ...
        $this->server->getLogger()->info('Level loading: ' . $this->displayName . '...');
          ...
          foreach($this->getPlayers() as $player){
            ...
            if(count($entities) < 10){
              $x = mt_rand($player->getX() - 20, $player->getX() + 20);
                ...
                $guard = $this->getServer()->getPluginManager()->getPlugin('RGuard');
                if ($guard !== null) {
                  if (!$guard->regionHere((int)$pos->x, (int)$pos->y, (int)$pos->z, $this->getName())) {
    ...
    public function getFullLight(Vector3 $pos): int {
        return 0; //$this->getFullLightAt($pos->x, $pos->y, $pos->z);
    }
    ...
    public function computeSkyLightReduction() : int{
        ...
        return 0;// (int) ($percentage * 11);
    }

     

    public function getSkyLightReduction() : int{
        return 0; //$this->skyLightReduction;
    }
