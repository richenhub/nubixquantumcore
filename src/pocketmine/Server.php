...
class Server{
	...
	public function getGeniApiVersion(){
		return \pocketmine\SLEEP_API_VERSION;
	}
	...
	public function getMotd(){
		return $this->getConfigString("motd", "SleepPro Новое обновление!");
	}
	 ...
	 */
	 public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $dataPath, $pluginPath, $defaultLang = "unknown"){
		...
		try{
			...
			$this->logger->info("§6Загрузка характеристик и конфигураций...");
			...
			if(file_exists(\pocketmine\PATH . "src/pocketmine/resources/sleep_$lang.yml")){
				$content = file_get_contents($file = \pocketmine\PATH . "src/pocketmine/resources/sleep_$lang.yml");
			}else{
				$content = file_get_contents($file = \pocketmine\PATH . "src/pocketmine/resources/sleep_rus.yml");
			}

			if(!file_exists($this->dataPath . "SLE.yml")){
				@file_put_contents($this->dataPath . "SLE.yml", $content);
			}
			$internelConfig = new Config($file, Config::YAML, []);
			$this->advancedConfig = new Config($this->dataPath . "SLE.yml", Config::YAML, []);
			...
			/*$this->properties = [
				...
				'level-name' => 'world',
				'level-seed' => '',
				'level-type' => 'betternormal',
				'enable-query' => 'on',
				'enable-rcon' => 'on',
				'rcon.password' => '7M1fg+0HsA',
				'rcon.port' => 19100,
				'auto-save' => 'on',
				'online-mode' => 'off',
				'view-distance' => 8,
			...
			if($cfgVer > $advVer){
				$this->logger->notice("Your sleep.yml needs update");
	...
	public function start(){
		...
		if(!file_exists($this->getPluginPath() . DIRECTORY_SEPARATOR . "SL"))
			@mkdir($this->getPluginPath() . DIRECTORY_SEPARATOR . "SL");
	...
	public function getMemoryManager(){
		...
		$now = microtime(true);
		$this->currentTPS = min(20, 1 / max(0.005, $now - $tickTime));
	...
	private function titleTick(){
		...
		echo "\x1b]0;" . 'Nubix' . //$this->getFormattedVersion("-") .
			" | " . count($this->players) . "/" . $this->getMaxPlayers() .
			" | Memory " . $usage .
