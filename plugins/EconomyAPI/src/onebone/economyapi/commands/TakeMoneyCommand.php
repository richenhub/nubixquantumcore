    ...
    public function exec(CommandSender $sender, array $params)
    {
        ...
        if($amount <= 0) {
            $sender->sendMessage($this->getPlugin()
                                      ->getMessage("takemoney-invalid-number", $sender->getName()));
            return true;
        }
        ...
        $result = $this->getPlugin()
                       ->reduceMoney($player, $amount, false, "TakeMoneyCommand");
        $output = "";
        switch($result) {
            case EconomyAPI::RET_SUCCESS:
                $output .= $this->getPlugin()
                                ->getMessage("takemoney-took-money", $sender->getName(),
                                    [$player, $amount, "%3", "%4"]);
                break;
            case EconomyAPI::RET_INVALID:
                $output .= $this->getPlugin()
                                ->getMessage("takemoney-player-lack-of-money", $sender->getName(), [
                                    $player, $amount, $this->getPlugin()
                                                           ->myMoney($player), "%4",
                                ]);
                break;
            default:
                $output .= $this->getPlugin()
                                ->getMessage("takemoney-failed", $sender->getName());
        }
