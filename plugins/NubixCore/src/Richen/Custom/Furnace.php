...
class Furnace extends Spawnable implements InventoryHolder, Container, Nameable {
	...
	public function __construct(Level $level, CompoundTag $nbt){
        ...
        if (!$this instanceof \pocketmine\tile\Furnace) return;
	...
	public function onUpdate(){
		...
		if($this->namedtag['BurnTime'] > 0){
			...
			$this->namedtag->BurnTicks = new ShortTag('BurnTicks', (int) ceil(($this->namedtag['BurnTime'] / $this->namedtag['MaxTime'] * 50)));
			...
			if($smelt instanceof FurnaceRecipe and $canSmelt){
			    ...
			    if($name == "§0§dСупер-печка") {
			        $this->namedtag->CookTime = new ShortTag('CookTime', $this->namedtag['CookTime'] + 25);
			    }
			    
			    if($name == "§1§dСупер-печка") {
			        $this->namedtag->CookTime = new ShortTag('CookTime', $this->namedtag['CookTime'] + 12);
			    }
			    
			    if($name != "§0§dСупер-печка" and $name != "§1§dСупер-печка") {
			        $this->namedtag->CookTime = new ShortTag('CookTime', $this->namedtag['CookTime'] + 11);
			    }
                echo ' aaakek ';
	...
	public function getSpawnCompound(){
		$nbt = new CompoundTag('', [
			new StringTag('id', \pocketmine\tile\Tile::FURNACE),
